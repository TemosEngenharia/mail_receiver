...
def parsemail(_mailfile):
    ...
    if 'reports@latam.officeTrack.com' in mail.from_:
        ...
        destination = source.replace('/manual/', '/ServiceNow/')
        logger.info('ServiceNow: Destination is %s' % destination)
        rename(source, destination)
    else:
        destination = source.replace('/manual/', '/Error/')
...
def main():
    while True:
        try:
            ...
            logger.error("Mailfile %s, can't be parsed" % mailfile.replace('/manual/', '/Error/'))
            destination = mailfile.replace('/manual/', '/Error/')
